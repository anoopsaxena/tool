...
SET mainScriptPath=D:\ismometricFiles\IsometricToolEngine\CrosscheckAlertNotification\main.py
...
SET logFile=%logDir%\CrosscheckNotification_log_%currentDateTime%.txt

:: Debug: Confirm batch file starts
echo Running main.py >> "%logFile%"

:: Check if the main script file exists and run it
IF EXIST "%mainScriptPath%" (
    "%pythonExe%" "%mainScriptPath%" > "%logFile%" 2>&1
    IF %ERRORLEVEL% EQU 0 (
